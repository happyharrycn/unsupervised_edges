function [E,O,inds,segs] = edgesDetect( I, Ig, model )
...
%  Ig         - [h x w x 3] color input image for sharpen
...
% Code written by Piotr Dollar, 2014. Modified by Yin Li, 2015
...
assert(size(I,1)==size(Ig,1) && size(I, 2)==size(Ig, 2));
...
if( opts.multiscale )
  ...
  for i=1:k, s=ss(i); I1=imResample(I,s); Ig1=imResample(Ig, s);
    if(nargout<4), [E1,~,inds{i}]=edgesDetect(I1, Ig1, model);
    else [E1,~,inds{i},segs{i}]=edgesDetect(I1, Ig1, model); end
